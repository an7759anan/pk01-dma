...
void *map_uncached_mem(MEM_MAP *mp, int size)
{
    ...
    ret = (mp->h = alloc_vc_mem(mp->fd, mp->size, DMA_MEM_FLAGS)) > 0 &&
        ...
        ? mp->virt : 0;
    // printf("VC mem handle %u, phys %p, virt %p, bus %p, size %u\n", mp->h, BUS_PHYS_ADDR(mp->bus), mp->virt, mp->bus, mp->size);
    return(ret);
}
...
{
    volatile uint32_t *reg = REG64(gpio_regs, GPIO_GPPUDCLK0) + pin / 32;

    *REG64(gpio_regs, GPIO_GPPUD) = pull;
    usleep(2);
    *reg = pin << (pin % 32);
    usleep(2);
    *REG64(gpio_regs, GPIO_GPPUD) = 0;
    *reg = 0;
}
...
{
    volatile uint32_t *reg = REG64(gpio_regs, GPIO_MODE0) + pin / 10, shift = (pin % 10) * 3;
...
{
    volatile uint32_t *reg = REG64(gpio_regs, val ? GPIO_SET0 : GPIO_CLR0) + pin/32;
    *reg = 1 << (pin % 32);
}
...
{
    volatile uint32_t *reg = REG64(gpio_regs, GPIO_LEV0) + pin/32;
    return (((*reg) >> (pin % 32)) & 1);
}
...
void *map_segment(void *addr, int size)
{
    ...
    mem = mmap(0, size, PROT_WRITE|PROT_READ, MAP_SHARED, fd, (uint64_t)addr);
    close(fd);
#if DEBUG
    printf("Map %p -> %p\n", (void *)addr, mem);
#endif
...
{
    *REG64(dma_regs, DMA_ENABLE) |= (1 << chan);
    *REG64(dma_regs, DMA_REG(chan, DMA_CS)) = 1 << 31;
}

// Start DMA, given first control block
void start_dma(MEM_MAP *mp, int chan, DMA_CB *cbp, uint32_t csval)
{
    *REG64(dma_regs, DMA_REG(chan, DMA_CONBLK_AD)) = MEM_BUS_ADDR(mp, cbp);
    *REG64(dma_regs, DMA_REG(chan, DMA_CS)) = 2;        // Clear 'end' flag
    *REG64(dma_regs, DMA_REG(chan, DMA_DEBUG)) = 7;     // Clear error bits
    *REG64(dma_regs, DMA_REG(chan, DMA_CS)) = 1|csval;  // Start DMA
}

// Return remaining transfer length
uint32_t dma_transfer_len(int chan)
{
    return(*REG64(dma_regs, DMA_REG(chan, DMA_TXFR_LEN)));
}

// Check if DMA is active
uint32_t dma_active(int chan)
{
    return((*REG64(dma_regs, DMA_REG(chan, DMA_CS))) & 1);
}
...
{
    volatile uint32_t *p=REG64(dma_regs, DMA_REG(chan, DMA_CS));
    int i=0;
...
{
    stop_pwm();
    if (*REG64(pwm_regs, PWM_STA) & 0x100)
    {
        printf("PWM bus error\n");
        *REG64(pwm_regs, PWM_STA) = 0x100;
    }
#if USE_VC_CLOCK_SET
    set_vc_clock(mbox_fd, PWM_CLOCK_ID, freq);
#else
    int divi=CLOCK_HZ / freq;
    *REG64(clk_regs, CLK_PWM_CTL) = CLK_PASSWD | (1 << 5);
    while (*REG64(clk_regs, CLK_PWM_CTL) & (1 << 7)) ;
    *REG64(clk_regs, CLK_PWM_DIV) = CLK_PASSWD | (divi << 12);
    *REG64(clk_regs, CLK_PWM_CTL) = CLK_PASSWD | 6 | (1 << 4);
    while ((*REG64(clk_regs, CLK_PWM_CTL) & (1 << 7)) == 0) ;
#endif
    usleep(100);
    *REG64(pwm_regs, PWM_RNG1) = range;
    *REG64(pwm_regs, PWM_FIF1) = val;
#if PWM_OUT
...
void start_pwm(void)
{
    *REG64(pwm_regs, PWM_CTL) = PWM_CTL_USEF1 | PWM_ENAB;
}
